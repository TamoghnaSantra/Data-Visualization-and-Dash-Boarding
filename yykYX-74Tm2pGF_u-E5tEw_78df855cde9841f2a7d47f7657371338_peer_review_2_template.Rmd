...
combined_data <- left_join(my_world_map, some_data_values, by='region')
ggplot(data=combined_data, mapping = aes(x=long, y = lat, group = group, fill = Score))+
...
countries <- filter(world, region == 'Costa Rica'|region == 'Panama'|region == 'Nicaragua')
head(maps::world.cities)
my_cities <- maps::world.cities
big_cities <- filter(my_cities, country.etc == 'Costa Rica' & pop > 40000 |country.etc == 'Panama' & pop > 40000 | country.etc == 'Nicaragua' & pop > 40000)
Measurement1 <- tibble(Measurement)
my_big_cities <- add_column(big_cities, Measurement1)
ggplot (data = countries, mapping = aes(x=long, y = lat, group = group))+
  geom_polygon(color='black', fill = 'white')+
  geom_point(data = my_big_cities, aes(x=long, y = lat, group = NULL, color = Measurement), size = 5)+
